...
=== TEST 31: proof header missing alg — 401, no runtime error
--- config
    location /t {
        content_by_lua_block {
            local h = require("lib.dpop")
...
=== TEST 32: proof header alg is not a string — 401, no runtime error
--- config
    location /t {
        content_by_lua_block {
            local h = require("lib.dpop")
...
=== TEST 33: proof header is not a JSON object — 401, no runtime error
--- config
    location /t {
        content_by_lua_block {
            local h = require("lib.dpop")
...
=== TEST 34: proof payload is not a JSON object — 401, no runtime error
--- config
    location /t {
        content_by_lua_block {
            local h = require("lib.dpop")
